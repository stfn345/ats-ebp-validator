...
{ 
   ...
   int i = 0; 
   while ((bs_read_u1(b) == 0) && (i < 32) && !bs_eof(b)) i++; 
...
{ 
   ...
   if (v <= 0) bs_write_ue(b, -(v << 1)); 
   else bs_write_ue(b, (v << 1) - 1);
}
...
{ 
   ...
   while (n--) bs_skip_u1(b);
}

static inline uint64_t bs_read_ull(bs_t *b, int n) 
{ 
   if (!b || bs_eof(b) || n < 1) return 0; 
   
   uint64_t r = 0; 
   while (n--) r |= ((uint64_t)bs_read_u1(b) << n); 
   return r;
}

static inline void bs_write_ull(bs_t *b, int n, uint64_t v) 
{ 
   if (!b || bs_eof(b) || n < 1) return; 
   
   while (n--) bs_write_u1(b, (uint32_t)(v >> n) & 0x1);
}
...
{ 
   ...
   while (n--) bs_write_u8(b, (v >> (n << 3)) & 0xFF);
}
...
{ 
   ...
   {
      b->p += len; 
      if (num_ch > 0) memset(b->p, v, num_ch); 
      else num_ch = 0;
   }
