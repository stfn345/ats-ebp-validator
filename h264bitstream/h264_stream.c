 ...
 * @return -1 on error, 0 for unsupported (ignored) NAL unit types, or size of read NAL unit
 */
int read_nal_unit(h264_stream_t* h, uint8_t* buf, int size) {
    ...
    if ((b = bs_new(rbsp_buf, rbsp_size)) == NULL)
    	goto read_nal_unit_wrap_up;
    ...
    switch ( nal->nal_unit_type )
    {
        ...
        default:
        	ret = 0; // unsupported, yet not error
        	goto read_nal_unit_wrap_up;
    }
...
void read_slice_layer_rbsp(h264_stream_t* h,  bs_t* b)
{
    ...
    if (bs_eof(b)) // there might be no bits left beyond the header
    	return;
...
int write_nal_unit(h264_stream_t* h, uint8_t* buf, int size)
{
    ...
    int nal_size = size, ret = -1, rbsp_size = size;
    uint8_t* rbsp_buf = (uint8_t*)calloc(1, rbsp_size);
    bs_t* b = 0;

    if (0) {
    	int rc = nal_to_rbsp(buf, &nal_size, rbsp_buf, &rbsp_size);
    	if (rc < 0) // handle conversion error
    		goto write_nal_unit_wrap_up;
    }
...
int read_debug_nal_unit(h264_stream_t* h, uint8_t* buf, int size)
{
    ...
    int nal_size = size, ret = -1, rbsp_size = size;
    uint8_t* rbsp_buf = (uint8_t*)calloc(rbsp_size, 1);
    bs_t* b = 0;

    if (1) {
    	int rc = nal_to_rbsp(buf, &nal_size, rbsp_buf, &rbsp_size);
    	if (rc < 0) // handle conversion error
    		goto read_debug_nal_unit_wrap_up;
    }
...
void read_debug_access_unit_delimiter_rbsp(h264_stream_t* h, bs_t* b)
{
    printf("%d.%d: ", b->p - b->start, b->bits_left); h->aud->primary_pic_type = bs_read_u(b, 3); 
    printf("h->aud->primary_pic_type: %d \n", h->aud->primary_pic_type); 
    read_debug_rbsp_trailing_bits(h, b);
}
